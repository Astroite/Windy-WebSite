...
[Windborne（风信来客）](https://github.com/Astroite) 的官方宣传落地页 —— 由真实天气驱动、常驻 Windows 桌面的低压力放置收集游戏。现已在 [Steam](https://store.steampowered.com/app/4947380/_/) 上线。

零构建、零依赖的静态站点：一份 `index.html` + `css/` + `js/`，剪纸拼贴 / 粉彩气象地图美术风格与主项目一致，双语（默认中文，可切换英文）。页面围绕真实天气玩法循环、**215 种天象访客**、**21 种牧场住客**、Journey 统一进度和低打扰桌面体验展开。

## 目录结构

```
index.html            单页落地页（导航 / hero / 玩法循环 / 特色 / 访客画廊 / 天气 / 牧场 / Journey / 地点 / Steam / 页脚）
...
assets/art/           剪纸主视觉与精选访客插画，以及来自当前产品的天气 / 牧场画面
tools/                构建脚本（见下）
```
...
# 2) 用 PaperArt 生成技能重出剪纸风主视觉 / 访客插画（异步，读技能 .env 里的 UAT key）
...
- `assets/art/ranch.jpg`：从相邻主项目的当前浏览器预览截取，只展示正式牧场界面与目录内住客，不包含个人地点、调试 HUD 或玩家存档信息；不由 `gen_all_art.py` 覆盖。
