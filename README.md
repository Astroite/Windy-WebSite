...
[Windborne（风信来客）](https://github.com/Astroite) 的官方宣传落地页 —— 自绘卡通天气壁纸 + 天象访客收集玩法。
...
js/site.js            画廊渲染、稀有度筛选、中英切换、滚动出现、生成图注入（零依赖）
data/visitors.json    215 个访客的 {id,label,labelEn,rarity,art}，由生成器产出
assets/visitors/      每个访客一张代表 sprite（从主项目复制）
...
美术与数据都从主项目 `WindyWallpaper` 派生，脚本可重跑（默认假设二者是相邻目录：`F:\Projects\As\WindyWallpaper` 与 `F:\Projects\As\WindyWebSite`）。

```bash
# 1) 提取访客元数据 + 复制代表 sprite（读 apps/wallpaper/data/visitors.js）
